...
epl_results_selected <- df |> 
  select(home_team, away_team, home_goals, away_goals, wk) |> 
...
results_data_to_plot <- all_results |> 
  ...
  arrange(team) |> ungroup() |> 
  relocate(team) |> 
  select(match_week, team, opponent, team_goals, opponent_goals, result_text) 
...
standings <- fb_season_team_stats("ENG", "M", 2023, "1st", "league_table") |> 
  mutate(team = Squad) |> 
  mutate(rank = as.numeric(Rk)) |> arrange(rank) |> select(team, rank)
```
...
```{r}
cols <- c("win" = "#A0BCD6", "draw" = "grey", "loss" = "#ef9a9a")

ggplot(results_data_to_plot, aes(factor(match_week), team, fill = result_text)) +
      geom_tile(color = "white",
                lwd = .5,
                linetype = 1,
                height = 0.4) +
      ...
      theme(
        ...
        panel.background = element_blank()) +
  scale_fill_manual(values = cols) +
 scale_y_discrete(limits = rev(levels(results_data_to_plot$team) ))
  
```

TO DO

fuzzy join league standings and plot data
